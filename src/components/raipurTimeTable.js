...
export default function RaipurTimeTable({ onlyRaipur, sehriAudioRef, iftariAudioRef }) {
    ...
    // Update soundEnabled ref when localStorage changes
    useEffect(() => {
        const checkSound = () => {
            const enabled = typeof localStorage !== 'undefined' && localStorage.getItem('ramzan_sound_enabled') === 'true';
        ...
        const check = () => {
                ...
                if (playedRef.current.sehri !== key && persisted !== key) {
                    ...
                    try {
                        if (soundEnabledRef.current) {
                            console.log('Playing Raipur sehri sound');
            ...
            if (iftariTime && isSameMinute(now, iftariTime)) {
                ...
                if (playedRef.current.iftari !== key && persisted !== key) {
                    ...
                    try {
                        if (soundEnabledRef.current) {
                            console.log('Playing Raipur iftari sound');
        ...
        <>
            ...
            <div className="mt-6 hidden md:grid md:grid-cols-2 gap-4">
                ...
                <div className="overflow-x-auto">
                    <table className="table table-compact w-full divide-y divide-primary">
                        <thead>
                            <tr>
                                <th className="bg-primary text-primary-content text-center">Roza</th>
                                <th className="bg-primary text-primary-content text-center">Date</th>
                                <th className="bg-primary text-primary-content text-center">Day</th>
                                <th className="bg-primary text-primary-content text-center">Sehri</th>
                                <th className="bg-primary text-primary-content text-center">Iftar</th>
                            </tr>
                        </thead>
