...
// ─── Field Card ───────────────────────────────────────────────────────────────
function FieldCard({ field, index, total, onUpdate, onDelete, onMoveUp, onMoveDown }) {
    const [expanded, setExpanded] = useState(false);
    const ft = getFieldTypeInfo(field.type);

    return (
        <div className={`bg-white border rounded-xl shadow-sm overflow-hidden transition ${expanded ? "border-violet-300" : "border-gray-200"}`}>
            {/* Header row */}
